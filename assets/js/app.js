...
async function openGameDetail(gameId) {
    ...
    renderGameDetailContentV6(game);
...
function renderGameDetailContentV3(game) {
    ...
    const shortDescription = description.length > 280 ? `${description.slice(0, 280).trim()}...` : description;

    document.getElementById('gameDetailHero').innerHTML = `
        <img src="${escapeHtml(game.backgroundUrl || game.coverUrl)}" alt="" class="game-detail-hero-bg"
             onerror="this.style.background='var(--bg-elevated)'">
        <div class="gd-v3-hero-scrim"></div>
        <div class="gd-v3-hero-shell">
            <div class="gd-v3-cover-wrap" onclick="openScreenshotLightbox('${escapeHtml(game.id)}')">
                <img src="${escapeHtml(game.coverUrl)}" alt="${escapeHtml(game.title)}" class="game-detail-cover gd-v3-cover"
                     onerror="this.style.background='var(--bg-elevated)'">
                ${(game.screenshots && game.screenshots.length > 0) ? `<div class="game-detail-cover-badge">Shots ${game.screenshots.length}</div>` : ''}
            </div>
            <div class="gd-v3-headline">
                <div class="gd-v3-kicker-row">
                    <span class="gd-v3-kicker">Game Detail</span>
                    <span class="gd-v3-year">${game.releaseYear || 'TBA'}</span>
                </div>
                <h2 class="game-detail-title gd-v3-title">${escapeHtml(game.title)}</h2>
                <p class="gd-v3-summary">${escapeHtml(shortDescription)}</p>
                <div class="game-detail-genres gd-v3-genres">
                    ${game.genres.map(g => `<span class="game-detail-genre-tag">${escapeHtml(g)}</span>`).join('')}
                </div>
                <div class="gd-v3-meta-line">
                    <span class="game-detail-developer">${escapeHtml(developer)}</span>
                    ${game.rating > 0
                        ? `<span class="game-detail-rating-large ${ratingClass}">${starSvg} ${game.rating}/100</span>`
                        : `<span class="game-detail-rating-large medium">Skor bekleniyor</span>`}
    ...
    document.getElementById('gameDetailInfo').innerHTML = `
        <div class="gd-v3-layout">
            <div class="gd-v3-main">
                <section class="gd-v3-panel gd-v3-panel--story">
                    <div class="gd-v3-panel-head">
                        <span>Genel Bakis</span>
                        <strong>Oyunu Neden Onemsemelisin?</strong>
                    </div>
                    <div class="gd-desc-box" id="gdDescBox" onclick="toggleGameDesc()">
                        <p class="gd-desc-text" id="gdDescText">${escapeHtml(description)}</p>
                        <div class="gd-desc-fade"></div>
                ...
                ${screenshots.length > 0 ? `
                    <section class="gd-v3-panel gd-v3-panel--gallery">
                        <div class="gd-v3-panel-head">
                            <span>Medya Seridi</span>
                            <strong>Gorsel Anlar</strong>
                        </div>
                        <div class="gd-v3-gallery">
                            ${screenshots.map((shot, index) => `
                                <button class="gd-v3-shot" onclick="event.stopPropagation();openScreenshotLightbox('${escapeHtml(game.id)}')" aria-label="Galeriyi ac ${index + 1}">
                                    <img src="${escapeHtml(shot)}" alt="${escapeHtml(game.title)} screenshot ${index + 1}">
                                </button>
                            `).join('')}
                        </div>
                ...
                ${game.tags && game.tags.length > 0 ? `
                    <section class="gd-v3-panel gd-v3-panel--tags">
                        <div class="gd-v3-panel-head">
                            <span>Kesif</span>
                ...
                ${game.systemRequirements && game.systemRequirements.length > 0 ? `
                    <section class="gd-v3-panel gd-v3-panel--sysreq">
                        <div class="gd-v3-panel-head">
            ...
            <aside class="gd-v3-side">
                <section class="gd-v3-panel gd-v3-panel--stats">
                    <div class="gd-v3-panel-head">
                        <span>Skor Kartlari</span>
                        <strong>Hizli Veriler</strong>
                    </div>
                    <div class="gd-v3-stat-stack">
                        <div class="gd-v3-stat-card">
                ...
                <section class="gd-v3-panel gd-v3-panel--meta">
                    <div class="gd-v3-panel-head">
                        <span>Baglantilar</span>
                        <strong>Studo ve Platform</strong>
                    </div>
                    <div class="gd-meta-block">
                        <div class="gd-meta-label">Platformlar</div>
                        <div class="gd-platforms">
            ...
            : '')}
                ...
                <section class="gd-v3-panel gd-v3-panel--market">
                    <div class="gd-v3-panel-head">
                        <span>Pazar</span>
...
function renderGameDetailContentV5(game) {
    const ratingClass = getRatingClass(game.rating);
    const starSvg = '<svg viewBox="0 0 24 24" fill="currentColor" stroke="none" width="16" height="16"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>';
    const description = game.description || 'Aciklama yukleniyor...';
    const developer = game.developer || 'Yukleniyor...';
    ...
    document.getElementById('gameDetailHero').innerHTML = `
        <img src="${escapeHtml(game.backgroundUrl || game.coverUrl)}" alt="" class="game-detail-hero-bg"
             onerror="this.style.background='var(--bg-elevated)'">
        <div class="gd-v5-hero-shade"></div>
        <div class="gd-v5-hero-shell">
            ...
            <div class="gd-v5-hero-card">
                <div class="game-detail-cover-wrapper gd-v5-cover-wrap" onclick="openScreenshotLightbox('${escapeHtml(game.id)}')">
                    <img src="${escapeHtml(game.coverUrl)}" alt="${escapeHtml(game.title)}" class="game-detail-cover gd-v5-cover"
                         onerror="this.style.background='var(--bg-elevated)'">
                    ${(game.screenshots && game.screenshots.length > 0) ? `<div class="game-detail-cover-badge">Shots ${game.screenshots.length}</div>` : ''}
                </div>
                <div class="gd-v5-quickstats">
                    <div class="gd-v5-quickstat"><span>Metacritic</span><strong>${game.rating > 0 ? game.rating : '—'}</strong></div>
                    <div class="gd-v5-quickstat"><span>Kullanici</span><strong>${game.rawRating ? game.rawRating.toFixed(1) : '—'}</strong></div>
                    <div class="gd-v5-quickstat"><span>Sure</span><strong>${game.playtime || '—'}</strong></div>
                </div>
            </div>
        </div>
    `;
    ...
    document.getElementById('gameDetailInfo').innerHTML = `
        <div class="gd-v5-layout">
            <main class="gd-v5-main">
...
function renderGameDetailContentV6(game) {
    const ratingClass = getRatingClass(game.rating);
    const starSvg = '<svg viewBox="0 0 24 24" fill="currentColor" stroke="none" width="16" height="16"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>';
    const description = game.description || 'Aciklama yukleniyor...';
    const developer = game.developer || 'Yukleniyor...';
    const publisher = game.publisher || 'Yukleniyor...';
    const displayPlatforms = game.allPlatforms && game.allPlatforms.length > 0 ? game.allPlatforms : game.platforms;
    const summary = description.length > 240 ? `${description.slice(0, 240).trim()}...` : description;

    document.getElementById('gameDetailHero').innerHTML = `
        <img src="${escapeHtml(game.backgroundUrl || game.coverUrl)}" alt="" class="game-detail-hero-bg"
             onerror="this.style.background='var(--bg-elevated)'">
        <div class="gd-v6-hero-shade"></div>
        <div class="gd-v6-hero-shell">
            <div class="gd-v6-copy">
                <div class="gd-v6-topline">
                    <span class="gd-v6-badge">Kesfet</span>
                    <span class="gd-v6-badge gd-v6-badge--muted">${game.releaseYear || 'TBA'}</span>
                </div>
                <h2 class="game-detail-title gd-v6-title">${escapeHtml(game.title)}</h2>
                <p class="gd-v6-summary">${escapeHtml(summary)}</p>
                <div class="game-detail-genres gd-v6-genres">
                    ${game.genres.map(g => `<span class="game-detail-genre-tag">${escapeHtml(g)}</span>`).join('')}
                </div>
                <div class="gd-v6-meta">
                    ${game.rating > 0
                        ? `<span class="game-detail-rating-large ${ratingClass}">${starSvg} ${game.rating}/100</span>`
                        : `<span class="game-detail-rating-large medium">Skor bekleniyor</span>`}
                    ${game.esrbRating ? `<span class="game-detail-esrb-badge">${escapeHtml(game.esrbRating)}</span>` : ''}
            ...
            <div class="gd-v6-sidehero">
                <div class="game-detail-cover-wrapper gd-v6-cover-wrap" onclick="openScreenshotLightbox('${escapeHtml(game.id)}')">
                    <img src="${escapeHtml(game.coverUrl)}" alt="${escapeHtml(game.title)}" class="game-detail-cover gd-v6-cover"
                         onerror="this.style.background='var(--bg-elevated)'">
                </div>
                <div class="gd-v6-mini-stats">
                    <div class="gd-v6-mini-stat"><span>Metacritic</span><strong>${game.rating > 0 ? game.rating : '—'}</strong></div>
                    <div class="gd-v6-mini-stat"><span>Kullanici</span><strong>${game.rawRating ? game.rawRating.toFixed(1) : '—'}</strong></div>
                    <div class="gd-v6-mini-stat"><span>Sure</span><strong>${game.playtime || '—'}</strong></div>
                </div>
            </div>
        </div>
    `;
    ...
    document.getElementById('gameDetailInfo').innerHTML = `
        <div class="gd-v6-layout">
            <main class="gd-v6-main">
                <section class="gd-v6-panel">
                    <div class="gd-v6-panel-head">
                        <span>Aciklama</span>
                        <strong>Oyun Hakkinda</strong>
                    </div>
                    <div class="gd-desc-box" id="gdDescBox" onclick="toggleGameDesc()">
